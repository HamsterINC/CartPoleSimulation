...
class Cartpole_Balance(CartPoleRLTemplate):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_space = spaces.Box(low=np.array([-self.x_threshold, -np.inf, -1.0, -1.0, -np.inf], dtype=np.float32), high=np.array([self.x_threshold, np.inf, 1.0, 1.0, np.inf], dtype=np.float32), dtype=np.float32)
    ...
    def reward_assignment(self, state, action, terminated):
        if not terminated:
            reward = (np.cos(state[ANGLE_IDX]) + 1) / 2  # Normalized 0 to 1
            reward -= 0.001 * (state[ANGLED_IDX]**2)   # Penalty for excessive spinning

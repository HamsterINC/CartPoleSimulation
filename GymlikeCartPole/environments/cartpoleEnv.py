...
from GymlikeCartPole.environments.cartpoleSwingUp import Cartpole_Balance
...
class CartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    def __init__(
        ...
        if swingup == True:
            self.cartpole_rl = Cartpole_Balance()
        else:
    ...
    def _get_obs(self):
        return np.array([self.state[POSITION_IDX], self.state[POSITIOND_IDX], self.state[ANGLE_SIN_IDX], self.state[ANGLE_COS_IDX], self.state[ANGLED_IDX]], dtype=np.float32)
    ...
    def step(self, action):
        ...
        state_for_physics = self.state
        if state_for_physics.ndim == 1:
            state_for_physics = state_for_physics[np.newaxis, :]
    
        # 2. Now call the physics engine with the 2D state
        self.state = self.cartpole_rl.get_next_state(state_for_physics, action)
